    ...
    if (user) {
      ...
      if (!comment.rating) {
        const check = comment.emotions?.find(
          (item) =>
            item.user.toString() === userCurrentId && comment.id===item.id &&
            item.type === type &&
            item.iden === "child"
        );
        if (check) {
          ...
          >
            ...
            >
              {replies.length > 0 ? (
                !moreReply ? (
                  <>
                    <i className="fa-solid fa-caret-down"></i> Xem trả lời
                  </>
                ) : (
                  <>
                    <i className="fa-solid fa-caret-up"></i> Ẩn trả lời
                  </>
                )
              ) : (
                ""
              ...
              replies.map((reply, i) => (
                ...
                >
                  <CommentActions
                    ...
                    checkLikeActive={checkActive(reply, 'like')}
                    checkDisLikeActive={checkActive(reply, 'dislike')}
                  />
                </CommentCardItem>
              ))}
          </CommentCardItem>
